...
public sealed class DeepExecutionEngineTests
{
    ...
    [Fact]
    public async Task RunSelected_SynthesizesStableDotNetIdentityWhenRuntimeCatalogCannotMatchIt()
    {
        const string identity = "dotnet:test:v1:App.csproj:dotnet:type:Example.Tests:member:Works";
        ...
        Assert.Equal("App.csproj", selected.ProjectPath);
    }
    ...
    private sealed class DeepAdapter : ILanguageAdapter, IBuildPreparer, ITestDiscoverer, ISelectedTestExecutor, ITestObserver
    {
        ...
        public ValueTask<AdapterIndex> IndexAsync(AdapterIndexRequest request, CancellationToken cancellationToken)
        {
            ...
            return ValueTask.FromResult(new AdapterIndex([unit], [new ImpactEdge("unit:a", PlanIdentity, EvidenceKind.StaticDependency)], [new TestDescriptor(PlanIdentity, "A(1)", "xunit")]));
        }
        public ValueTask<MappingResult> MapAsync(AdapterMapRequest request, CancellationToken cancellationToken) => ValueTask.FromResult(MapRequested
            ? new MappingResult([new RequestedTest(PlanIdentity, "A(1)", "xunit", [], true)], [])
            : new MappingResult([], []));
